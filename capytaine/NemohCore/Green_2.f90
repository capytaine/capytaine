...
CONTAINS
  ...
  SUBROUTINE VNSFD &
    ...
    CALL COMPUTE_S2(XI(:), XJ(:), depth, wavenumber, FS(1), VS(:, 1))
    ...
    XI(3) = -X0I(3) - 2*depth
    XJ(3) =  X0J(3)
    CALL COMPUTE_S2(XI(:), XJ(:), depth, wavenumber, FS(2), VS(:, 2))
    ...
    XI(3) =  X0I(3)
    XJ(3) = -X0J(3) - 2*depth
    CALL COMPUTE_S2(XI(:), XJ(:), depth, wavenumber, FS(3), VS(:, 3))
    ...
    XI(3) = -X0I(3) - 2*depth
    XJ(3) = -X0J(3) - 2*depth
    CALL COMPUTE_S2(XI(:), XJ(:), depth, wavenumber, FS(4), VS(:, 4))
