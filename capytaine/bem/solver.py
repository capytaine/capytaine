...
class PrecondBEMSolver(BEMSolver):
    def solve(self, problem, keep_details=True):
        """Solve the linear potential flow problem using a blockJacobi +
        coarse correction preconditioner.
        ...
        S, K, *preconddata = self.engine.build_matrices(
            problem.body.mesh, problem.body.mesh,
            problem.free_surface, problem.water_depth, problem.wavenumber,
            self.green_function, problem.body.leaf_order,
            problem.body.path_to_leaf
        )
