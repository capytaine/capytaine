...
from capytaine.reference_bodies import generate_horizontal_cylinder, generate_clever_horizontal_cylinder
...
def full_resolution_Nemoh(nb_slices, nb_theta, omega_range):
    cylinder = generate_horizontal_cylinder(length=10.0, radius=1.0, nx=nb_slices+1, nr=0, ntheta=nb_theta+1)
...
def trans_Capytaine(nb_slices, nb_theta, omega_range):
    trans_cylinder = generate_clever_horizontal_cylinder(length=10.0, radius=1.0, nx=nb_slices+1, ntheta=nb_theta+1)
    trans_cylinder.translate_x(-5.0)
    trans_cylinder.translate_z(-2.0)
...
def trans_sym_Capytaine(nb_slices, nb_theta, omega_range):
    ring = generate_horizontal_cylinder(length=10.0/nb_slices, radius=1.0, nx=2, nr=0, ntheta=nb_theta+1)
