...
def test_solver_velocity_reconstruction():
    ...
    Y, Z = np.meshgrid(np.linspace(-3.0, 3.0, 40), np.linspace(-3.0, -0.01, 20))
    ...
    for ax, dof in zip(axs.ravel(), ["Sway", "Heave"]):
        y_velocities = -velocities[dof][:, 1].reshape(Y.shape).real
        z_velocities = -velocities[dof][:, 2].reshape(Z.shape).real
        ax.quiver(Y, Z, y_velocities, z_velocities, angles="xy")
        ax.axis('equal')
