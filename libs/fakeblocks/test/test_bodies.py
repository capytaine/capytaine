...
def test_cluster_bodies():
    centers = np.array([[0.0, 0.0, 0.0],
                        ...
                        [11.0, 0.0, 0.0],
                        [10.0, 1.0, 0.0]])
    meshes = [mesh_sphere(center=c, name=str(c)) for c in centers]
    ...
    assert isinstance(clustered_bodies.mesh, CollectionOfMeshes)
...
def test_mincing():
    body = cpt.FloatingBody(mesh=mesh_horizontal_cylinder(length=10, radius=0.5))
