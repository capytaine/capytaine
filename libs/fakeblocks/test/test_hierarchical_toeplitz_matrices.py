...
from fakeblocks.meshes.predefined import mesh_sphere, mesh_horizontal_cylinder, mesh_vertical_cylinder
from fakeblocks.meshes.symmetric import AxialSymmetricMesh, ReflectionSymmetricMesh, TranslationalSymmetricMesh
...
def test_floating_sphere(depth, omega, method):
    ...
    sphere_mesh = mesh_sphere(radius=1.0, resolution=(reso, 4*reso)).immersed_part()
    full_sphere = cpt.FloatingBody(mesh=sphere_mesh)
    ...
    half_sphere_mesh = sphere_mesh.extract_faces(
        np.where(full_sphere.mesh.faces_centers[:, 1] > 0)[0],
        name="half_sphere_mesh")
    two_halves_sphere = FloatingBody(mesh=ReflectionSymmetricMesh(half_sphere_mesh, xOz_Plane))
    ...
    clever_sphere_mesh = mesh_sphere(radius=1.0, resolution=(reso, 4*reso), axial_symmetry=True).immersed_part()
    clever_sphere = cpt.FloatingBody(mesh=clever_sphere_mesh)
...
def test_two_vertical_cylinders():
    ...
    mesh = mesh_vertical_cylinder(length=3, radius=0.5, center=(-distance/2, -1, 0), resolution=(3, 8, 8)).immersed_part()
    buoy = cpt.FloatingBody(mesh=mesh, name="cylinder")
    buoy.add_translation_dof(name="Sway")

    two_buoys = join_bodies(buoy, buoy.translated_x(distance, name="other_cylinder"))
    two_buoys.mesh = two_buoys.mesh.symmetrized(yOz_Plane)  # Use a ReflectionSymmetry as mesh...
...
def test_horizontal_cylinder(depth, method):
    ...
    trans_mesh = mesh_horizontal_cylinder(
        length=10.0,
        radius=1.0,
        center=(0, 0, -3.0),
        reflection_symmetry=False,
    ...
    trans_cylinder = cpt.FloatingBody(mesh=trans_mesh)
...
def test_low_rank_matrices(method):
    ...
    mesh = mesh_sphere(radius=radius, center=(0.0, 0.0, 0.0),
                       resolution=(int(perimeter*resolution/2), int(perimeter*resolution)),
                       axial_symmetry=False).immersed_part()
    buoy = cpt.FloatingBody(mesh, name="buoy")
    ...
    S, V = solver_with_sym.engine.build_matrices(two_distant_buoys.mesh, two_distant_buoys.mesh)
...
@pytest.mark.xfail  # No assemble_regular_array method constructed a TranslationalSymmetricMesh
...
def test_array_of_spheres(method, adjoint_double_layer):
    ...
    # Corner case
    dumb_array = assemble_regular_array(distance=5.0, nb_bodies=(1, 1))
    ...
    #
    array = assemble_regular_array(distance=4.0, nb_bodies=(3, 1))
